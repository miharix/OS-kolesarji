...
$pdf->SetTitle('Potrdila za mentorje');
$pdf->SetSubject('V1.0');
...
$mentorji=SQL_VsiMetnorji();
    while ($mentor = $mentorji->fetchArray(SQLITE3_NUM) ){
    	$kjeX=$pdf->GetX();
    	$kjeY=$pdf->GetY();
		...
		$pdf->SetFont('freeserif', 'B', 15,'',false);
    	$pdf->MultiCell(0, 8, 'Potrjujemo, da je bil/la '.$mentor[1], 0, 'C', 1, 1, '', '', true);
    	$pdf->MultiCell(0, 20, 'Mentor/ica '.$mentor[0]. ' tekmovalcem/kam', 0, 'C', 1, 1, '', '', true);
...
$pdf->Output('priznanja_mentorji.pdf', 'I');//D za vsiljen download I za odprtje v brskaniku
